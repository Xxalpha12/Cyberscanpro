...
def index():
    ...
    for s in enriched:
        raw_tgt = s["target"]
        tgt = raw_tgt.strip().lower()
        if tgt not in target_map:
            target_map[tgt] = {"name":raw_tgt.strip(),"scan_count":0,"latest_risk":"NONE"}
...
def targets_page():
    ...
    target_map = {}
    for s in sessions:
        # Group case-insensitively — "Bluewavetec.com" and "bluewavetec.com" are
        # the same target and were previously showing up as two separate rows.
        raw_name = s["target"]
        key = raw_name.strip().lower()
        if key not in target_map:
            target_map[key] = {
                "name": raw_name.strip(), "ip": None,
